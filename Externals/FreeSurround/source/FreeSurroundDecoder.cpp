...
DPL2FSDecoder::DPL2FSDecoder(channel_setup setup, unsigned int N,
    ...
    : N(N), wnd(N), inbuf(3 * N), setup(setup), samplerate(sample_rate),
      buffer_empty(true), initialized(false), lt(N), rt(N), dst(N),
      lf(N / 2 + 1), rf(N / 2 + 1), forward(kiss_fftr_alloc(N, 0, 0, 0)),
...
void DPL2FSDecoder::set_sample_rate(unsigned int v) {
  samplerate = v;
...
void DPL2FSDecoder::set_block_size(unsigned int v) {
