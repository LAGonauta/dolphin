...
unsigned int Mixer::MixSurround(float* samples, unsigned int num_samples)
{
  ...
  uint32_t needed_frames = m_surround_decoder.QueryFramesNeededForSurroundOutput(num_samples);

  // Mix() may also use m_scratch_buffer internally, but is safe because it alternates reads
  // and writes.
  uint32_t available_frames = Mix(m_scratch_buffer.data(), needed_frames);
